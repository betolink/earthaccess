...
### 3. Import Path Changes for Advanced Users
...
Search results now render as formatted HTML in Jupyter notebooks:

```python
# Automatic rich display
granules = earthaccess.search_data(short_name="ATL06", count=10)
granules  # Displays as formatted HTML table
```
...
| `pages()` | `SearchResults` | Iterate by pages |
